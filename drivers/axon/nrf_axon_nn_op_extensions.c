...
#define MAX_EXP_INPUT (31182) // the maximum input to exp before it saturates
	...
	for (uint16_t area_ndx = 0; area_ndx < area; area_ndx++, input_ptr++) {
		...
		for (uint16_t channel_ndx = 1; channel_ndx < base1_args->remaining_args.channel_cnt;
		...
		/* The Q11.12 value saturates at 2048. ln(2048) = 7.61283103, converted to q11.12 is
		  31182. Set the maximum value to this, and adjust all the other values accordingly.
		*/
		int32_t offset = max_value - MAX_EXP_INPUT;
		channel_ptr = input_ptr;
		for (uint16_t channel_ndx = 0; channel_ndx < base1_args->remaining_args.channel_cnt;
		     channel_ndx++, channel_ptr += area) {
			*channel_ptr -= offset;
		}
	}
	...
	for (uint16_t done_so_far = 0; done_so_far < total_elements;
	...
	for (uint16_t area_ndx = 0; area_ndx < area; area_ndx++, input_ptr++) {
		...
		if (sum == 0) {
			// Ideally we should not have a sum value of zero as we are offsetting the
			// input to the exponents in the first software step, in the off-case we do
			// get all zeros, the actual values are going to be very small floating
			// values which when quantized are equal to -128 setting sum=1 prevents
			// undefined results w/o affecting the final results
			sum = 1;
		}
		...
		for (uint16_t channel_ndx = 0; channel_ndx < base1_args->remaining_args.channel_cnt;
		     channel_ndx++, channel_ptr += area) {
			/**
			 * @FIXME!! SEE ABOUT SAVING THE *channel_ptr/sum term to input, then using
			 * axon to quantize saturate. Also, the below doesn't handle negative
			 * saturation.
			 */

			int64_t temp = (int64_t)(*channel_ptr) *
				       (int64_t)base1_args->remaining_args.output_multiplier;
			temp /= sum;
			temp += ((int64_t)base1_args->remaining_args.output_zeropoint
				 << base1_args->remaining_args.output_rounding);
			temp >>= base1_args->remaining_args.output_rounding;
			switch (base1_args->remaining_args.output_bytewidth) {
			case 1:
				(*((int8_t *)(base1_args->ptr_args.output) +
				   (area_ndx + (area * channel_ndx)))) =
					(int8_t)(temp > INT8_MAX   ? INT8_MAX
						 : temp < INT8_MIN ? INT8_MIN
								   : temp);
				break;
			case 2:
				(*((int16_t *)(base1_args->ptr_args.output) +
				   (area_ndx + (area * channel_ndx)))) =
					(int16_t)(temp > INT16_MAX   ? INT16_MAX
						  : temp < INT16_MIN ? INT16_MIN
								     : temp);
				break;
			case 4:
				(*((int32_t *)(base1_args->ptr_args.output) +
				   (area_ndx + (area * channel_ndx)))) =
					(int32_t)(temp > INT32_MAX   ? INT32_MAX
						   ...
						   NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE *args)
{
	...
	uint8_t input_extra_stride = (!base1_args->remaining_args.input_is_packed &&
				      base1_args->remaining_args.width & 1)
					     ? 1
					     : 0;
	uint8_t output_extra_stride = base1_args->remaining_args.output_bytewidth == 4
					      ? 0
					      : (4 - (base1_args->remaining_args.width & 3)) & 3;
	...
	for (uint16_t channel_ndx = 0; channel_ndx < base1_args->remaining_args.channel_cnt;
		...
		for (uint16_t row_ndx = 0; row_ndx < base1_args->remaining_args.height; row_ndx++) {
			for (uint16_t col_ndx = 0; col_ndx < base1_args->remaining_args.width;
			     col_ndx++, input_ptr++) {
				...
				case 1: // quantized output. scales between 0 and 1.
					...
					*output_ptr.i8 = scratch > 127 ? 127
							 : scratch < -128
								 ? -128
								 : (int8_t)scratch; // saturated
					output_ptr.i8++;
					break;
				case 4: // q1.30 output
	...
	for (uint16_t channel_ndx = 0; channel_ndx < base1_args->remaining_args.channel_cnt;
		...
		for (uint16_t row_ndx = 0; row_ndx < base1_args->remaining_args.height; row_ndx++) {
			for (uint16_t col_ndx = 0; col_ndx < base1_args->remaining_args.width;
				...
				scratch /= (float)(1 << 11); // input is q.12, multiply by 2 and
							     // convert to float
						   ...
						   NRF_AXON_PLATFORM_BITWIDTH_UNSIGNED_TYPE *args)
{
	...
	for (uint16_t chan_ndx = 0; chan_ndx < base2_args->remaining_args.output_channel_cnt;
	     chan_ndx++) { // channel
		for (uint16_t row_ndx = 0; row_ndx < base2_args->remaining_args.output_height;
		     row_ndx++) { // height
			for (uint16_t col_ndx = 0;
				...
				int xtf =
					row_ndx * (base2_args->remaining_args.output_width *
				...
				int old_idx = c_prime * (base2_args->remaining_args.input_height *
							 input_stride) +
					      h_prime * input_stride + w_prime;

				int new_idx = chan_ndx * (base2_args->remaining_args.output_height *
							  output_stride) +
