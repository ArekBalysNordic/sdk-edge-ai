				       ...
				       uint8_t type, uint16_t size, uint8_t *value)
{
	...
	if (clusterId == OnOff::Id && attributeId == OnOff::Attributes::OnOff::Id) {
		...
		/* LEDWidget::Set() uses k_timer_stop(). Run it on the app task thread only: this
		 * callback is invoked from the Matter/CHIP context; keyword control uses
		 * Nrf::PostTask as well. Concurrent k_timer_stop on the same LED corrupts the
		//  * kernel timeout list (see remove_timeout / bus fault on BFAR ~0x80…). */
		// const bool on = *value;
		// Nrf::PostTask([on] {
		// 	const bool value_invert = !on;
